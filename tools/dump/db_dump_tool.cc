...
bool DbDumpTool::Run(const DumpOptions& dump_options,
  ...
  rocksdb_silk::DB* dbptr;
  rocksdb_silk::Status status;
  std::unique_ptr<rocksdb_silk::WritableFile> dumpfile;
  char hostname[1024];
  int64_t timesec = 0;
  std::string abspath;
  char json[4096];
  ...
  status = rocksdb_silk::DB::OpenForReadOnly(options, dump_options.db_path, &dbptr);
  if (!status.ok()) {
  ...
  const std::unique_ptr<rocksdb_silk::DB> db(dbptr);

  status = env->NewWritableFile(dump_options.dump_location, &dumpfile,
                                rocksdb_silk::EnvOptions());
  if (!status.ok()) {
  ...
  rocksdb_silk::Slice magicslice(magicstr, 8);
  ...
  rocksdb_silk::Slice versionslice(versionstr, 8);
  ...
  rocksdb_silk::Slice infoslice(json, strlen(json));
  char infosize[4];
  rocksdb_silk::EncodeFixed32(infosize, (uint32_t)infoslice.size());
  rocksdb_silk::Slice infosizeslice(infosize, 4);
  ...
  const std::unique_ptr<rocksdb_silk::Iterator> it(
      db->NewIterator(rocksdb_silk::ReadOptions()));
  for (it->SeekToFirst(); it->Valid(); it->Next()) {
    char keysize[4];
    rocksdb_silk::EncodeFixed32(keysize, (uint32_t)it->key().size());
    rocksdb_silk::Slice keysizeslice(keysize, 4);
    ...
    char valsize[4];
    rocksdb_silk::EncodeFixed32(valsize, (uint32_t)it->value().size());
    rocksdb_silk::Slice valsizeslice(valsize, 4);
...
bool DbUndumpTool::Run(const UndumpOptions& undump_options,
  ...
  rocksdb_silk::DB* dbptr;
  rocksdb_silk::Status status;
  rocksdb_silk::Env* env;
  std::unique_ptr<rocksdb_silk::SequentialFile> dumpfile;
  rocksdb_silk::Slice slice;
  char scratch8[8];

  static const char* magicstr = "ROCKDUMP";
  static const char versionstr[8] = {0, 0, 0, 0, 0, 0, 0, 1};

  env = rocksdb_silk::Env::Default();
  ...
  uint32_t infosize = rocksdb_silk::DecodeFixed32(slice.data());
  ...
  status = rocksdb_silk::DB::Open(options, undump_options.db_path, &dbptr);
  if (!status.ok()) {
  ...
  while (1) {
    ...
    rocksdb_silk::Slice keyslice;
    rocksdb_silk::Slice valslice;

    status = dumpfile->Read(4, &slice, scratch8);
    if (!status.ok() || slice.size() != 4) break;
    keysize = rocksdb_silk::DecodeFixed32(slice.data());
    ...
    valsize = rocksdb_silk::DecodeFixed32(slice.data());
    ...
    status = db->Put(rocksdb_silk::WriteOptions(), keyslice, valslice);
    if (!status.ok()) {
  ...
  if (undump_options.compact_db) {
    status = db->CompactRange(rocksdb_silk::CompactRangeOptions(), nullptr, nullptr);
    if (!status.ok()) {
      fprintf(stderr,
