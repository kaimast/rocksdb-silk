...
TEST_F(LdbCmdTest, HexToString) {
  ...
  for (const auto& inPair : inputMap) {
    auto actual = rocksdb_silk::LDBCommand::HexToString(inPair.first);
    ...
    auto reverse = rocksdb_silk::LDBCommand::StringToHex(actual);
...
TEST_F(LdbCmdTest, HexToStringBadInputs) {
  ...
  for (const auto badInput : badInputs) {
    try {
      rocksdb_silk::LDBCommand::HexToString(badInput);
