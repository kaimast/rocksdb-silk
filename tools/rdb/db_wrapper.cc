...
Handle<Value> DBWrapper::Open(const Arguments& args) {
  ...
  std::vector<std::string> cfs = { rocksdb_silk::kDefaultColumnFamilyName };
  ...
  if (cfs.size() == 1) {
    db_wrapper->status_ = rocksdb_silk::DB::Open(
        db_wrapper->options_, db_file, &db_wrapper->db_);

    return scope.Close(Boolean::New(db_wrapper->status_.ok()));
  }

  std::vector<rocksdb_silk::ColumnFamilyDescriptor> families;

  for (std::vector<int>::size_type i = 0; i < cfs.size(); i++) {
    families.push_back(rocksdb_silk::ColumnFamilyDescriptor(
        cfs[i], rocksdb_silk::ColumnFamilyOptions()));
  }
...
Handle<Value> DBWrapper::Get(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ = db_wrapper->db_->Get(
        rocksdb_silk::ReadOptions(), key, &value);
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ = db_wrapper->db_->Get(
        rocksdb_silk::ReadOptions(), db_wrapper->columnFamilies_[cf], key, &value);
  } else {
    return scope.Close(Null());
  }
...
Handle<Value> DBWrapper::Put(const Arguments& args) {
  ...
  if (args[2]->IsUndefined()) {
    db_wrapper->status_  = db_wrapper->db_->Put(
      rocksdb_silk::WriteOptions(), key, value
    );
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ = db_wrapper->db_->Put(
      rocksdb_silk::WriteOptions(),
...
Handle<Value> DBWrapper::Delete(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ = db_wrapper->db_->Delete(
        rocksdb_silk::WriteOptions(), arg0);
  } else {
    if (!db_wrapper->HasFamilyNamed(arg1, db_wrapper)) {
      return scope.Close(Boolean::New(false));
    }
    db_wrapper->status_ = db_wrapper->db_->Delete(
        rocksdb_silk::WriteOptions(), db_wrapper->columnFamilies_[arg1], arg0);
  }
...
Handle<Value> DBWrapper::Dump(const Arguments& args) {
  HandleScope scope;
  std::unique_ptr<rocksdb_silk::Iterator> iterator;
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  std::string arg0      = *v8::String::Utf8Value(args[0]->ToString());

  if (args[0]->IsUndefined()) {
    iterator.reset(db_wrapper->db_->NewIterator(rocksdb_silk::ReadOptions()));
  } else {
    ...
    iterator.reset(db_wrapper->db_->NewIterator(
        rocksdb_silk::ReadOptions(), db_wrapper->columnFamilies_[arg0]));
  }
...
Handle<Value> DBWrapper::CreateColumnFamily(const Arguments& args) {
  ...
  rocksdb_silk::ColumnFamilyHandle* cf;
  db_wrapper->status_ = db_wrapper->db_->CreateColumnFamily(
      rocksdb_silk::ColumnFamilyOptions(), cf_name, &cf);
...
bool DBWrapper::AddToBatch(rocksdb_silk::WriteBatch& batch, bool del,
                           Handle<Array> array) {
...
Handle<Value> DBWrapper::WriteBatch(const Arguments& args) {
  ...
  db_wrapper->status_ = db_wrapper->db_->Write(rocksdb_silk::WriteOptions(), &batch);
...
Handle<Value> DBWrapper::CompactRangeDefault(const Arguments& args) {
  ...
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  rocksdb_silk::Slice begin     = *v8::String::Utf8Value(args[0]->ToString());
  rocksdb_silk::Slice end       = *v8::String::Utf8Value(args[1]->ToString());
