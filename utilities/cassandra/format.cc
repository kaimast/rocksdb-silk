...
void ColumnBase::Serialize(std::string* dest) const {
  rocksdb_silk::cassandra::Serialize<int8_t>(mask_, dest);
  rocksdb_silk::cassandra::Serialize<int8_t>(index_, dest);
}

std::shared_ptr<ColumnBase> ColumnBase::Deserialize(const char* src,
                                                    std::size_t offset) {
  int8_t mask = rocksdb_silk::cassandra::Deserialize<int8_t>(src, offset);
...
void Column::Serialize(std::string* dest) const {
  ...
  rocksdb_silk::cassandra::Serialize<int64_t>(timestamp_, dest);
  rocksdb_silk::cassandra::Serialize<int32_t>(value_size_, dest);
...
std::shared_ptr<Column> Column::Deserialize(const char *src,
  ...
  int8_t mask = rocksdb_silk::cassandra::Deserialize<int8_t>(src, offset);
  offset += sizeof(mask);
  int8_t index = rocksdb_silk::cassandra::Deserialize<int8_t>(src, offset);
  offset += sizeof(index);
  int64_t timestamp = rocksdb_silk::cassandra::Deserialize<int64_t>(src, offset);
  offset += sizeof(timestamp);
  int32_t value_size = rocksdb_silk::cassandra::Deserialize<int32_t>(src, offset);
...
void ExpiringColumn::Serialize(std::string* dest) const {
  ...
  rocksdb_silk::cassandra::Serialize<int32_t>(ttl_, dest);
}
...
std::shared_ptr<ExpiringColumn> ExpiringColumn::Deserialize(
  ...
  offset += value_size;
  int32_t ttl =  rocksdb_silk::cassandra::Deserialize<int32_t>(src, offset);
...
void Tombstone::Serialize(std::string* dest) const {
  ColumnBase::Serialize(dest);
  rocksdb_silk::cassandra::Serialize<int32_t>(local_deletion_time_, dest);
  rocksdb_silk::cassandra::Serialize<int64_t>(marked_for_delete_at_, dest);
}
...
std::shared_ptr<Tombstone> Tombstone::Deserialize(const char *src,
  ...
  offset += sizeof(index);
  int32_t local_deletion_time =
    rocksdb_silk::cassandra::Deserialize<int32_t>(src, offset);
  offset += sizeof(int32_t);
  int64_t marked_for_delete_at =
    rocksdb_silk::cassandra::Deserialize<int64_t>(src, offset);
  return std::make_shared<Tombstone>(
