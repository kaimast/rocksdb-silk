...
TEST_F(ColumnFamilyTest, CreateCFRaceWithGetAggProperty) {
  ...
  rocksdb_silk::port::Thread thread([&] { CreateColumnFamilies({"one"}); });
...
class TestComparator : public Comparator {
  int Compare(const rocksdb_silk::Slice& a, const rocksdb_silk::Slice& b) const override {
    return 0;
  }
  const char* Name() const override { return "Test"; }
  void FindShortestSeparator(std::string* start,
                             const rocksdb_silk::Slice& limit) const override {}
...
TEST_F(ColumnFamilyTest, AutomaticAndManualCompactions) {
  ...
  rocksdb_silk::port::Thread threads([&] {
    CompactRangeOptions compact_options;
    compact_options.exclusive_manual_compaction = false;
    ASSERT_OK(
        db_->CompactRange(compact_options, handles_[2], nullptr, nullptr));
...
TEST_F(ColumnFamilyTest, SameCFManualManualCompactions) {
  ...
  rocksdb_silk::port::Thread threads1([&] {
    CompactRangeOptions compact_options;
    compact_options.exclusive_manual_compaction = false;
    ASSERT_OK(
        db_->CompactRange(compact_options, handles_[1], nullptr, nullptr));
...
TEST_F(ColumnFamilyTest, CreateAndDropRace) {
  ...
  rocksdb_silk::SyncPoint::GetInstance()->SetCallBack("PersistRocksDBOptions:start",
                                                 [&](void* arg) {
  ...
  // and its comparator
  rocksdb_silk::port::Thread drop_cf_thread(DropSingleColumnFamily, this, 1, &comparators);
...
TEST_F(ColumnFamilyTest, LogSyncConflictFlush) {
  ...
  rocksdb_silk::port::Thread thread([&] { db_->SyncWAL(); });
